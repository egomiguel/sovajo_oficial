#ifndef SEGMENTATION_PLANE_LINE_H
#define SEGMENTATION_PLANE_LINE_H
...
namespace SPINE
{
	...
	{
		class SPINE_SEGMENTATION_EXPORT SpinePlane
		{
			//ax + by + cz + d = 0
		public:
			SpinePlane();
			...
			SpinePlane getPerpendicularPlane(const cv::Point3d& point1, const cv::Point3d& point2) const;

			cv::Point3d getInterceptionLinePoint(const SpineLine& a) const;
			...
			static SpinePlane getBestPlane(const std::vector<cv::Point3d>& pPoints);
