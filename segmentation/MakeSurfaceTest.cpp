...
namespace MySurface {
    ...
    std::vector<int> AlternativeDimensions = {28, 28, 1};
    std::vector<int> ImagePadding = {4, 4, 0};
    ...
    void CalculateContourNormal(vtkPolyData* inputPolyData, double outputNormal[3], int minimumContourSize)
    {
        ...
        {
            double contourNormal[3] = { 1, 0, 0 };//******************************************************now is default
            ...
            if (extract->GetOutput()->GetNumberOfPoints() > minimumContourSize)
            {
                ...
                //vtkAddonMathUtilities::FitPlaneToPoints(extract->GetOutput()->GetPoints(), plane);*********Find plane
    ...
    //Rotate contours to z axis ***** (it must be impruve)*****************************************************************
    ...
    double GetSpacingBetweenLines(vtkPolyData* inputROIPoints)
    {
        ...
        double defaultSliceThickness = 0.0;
    ...
    void TriangulateBetweenContours(vtkPolyData* inputROIPoints, vtkIdList* pointsInLine1, vtkIdList* pointsInLine2, vtkCellArray* outputPolygons)
    {
        ...
        for (int line1PointIndex = 0; line1PointIndex < numberOfPointsInLine1; ++line1PointIndex)
        {
            ...
            closestPointFromLine1ToLine2Ids[line1PointIndex] = GetClosestPoint(inputROIPoints, line1Point, pointsInLine2);
        }
        ...
        for (int line2PointIndex = 0; line2PointIndex < numberOfPointsInLine2; ++line2PointIndex)
        {
            ...
            closestPointFromLine2ToLine1Ids[line2PointIndex] = GetClosestPoint(inputROIPoints, line2Point, pointsInLine1);
        }
    ...
    void FixLine(vtkLine* oldLine, vtkLine* newLine)
    {
    ...
    void FixLines(vtkPolyData* oldLines, vtkPolyData* newLines)
    {
        if (!oldLines)
        {
            return;
    ...
    double ComputeError(vtkPoints* points, vtkIdList* lineIds, vtkIdType pointIndex)
    {
        if (!points)
        {
            return 0.0;
    ...
    void RemovePointDecimation(vtkIdList* originalIdList, vtkIdType pointId)
    {
    ...
    void MyCreateSmoothEndCapContour(vtkPolyData* inputROIPoints, vtkLine* inputLine, vtkCellArray* outputLines, double lineSpacing)
    {
        if (!inputROIPoints)
        {
            return;
        ...
        vtkNew<vtkImageData> blankImage;
        ...
        vtkSmartPointer<vtkPolyDataToImageStencil> polyDataToImageStencil = vtkPolyDataToImageStencil::New();
        ...
        while (numberOfVoxels > totalNumberOfVoxels / 2 && voxelDifference > 0)
        {
            ...
            vtkNew<vtkImageDilateErode3D> imageDilateErode3D;
    ...
    void CreateEndCapContour(vtkPolyData* inputROIPoints, vtkLine* inputLine, vtkCellArray* outputLines, double lineSpacing)
    {
        ...
        //EndCappingModes::Smooth
        MyCreateSmoothEndCapContour(inputROIPoints, inputLine, outputLines, lineSpacing);

        /*if (vtkVariant(GetConversionParameter(GetEndCappingParameterName())).ToInt() == EndCappingModes::Smooth)
        {
            CreateSmoothEndCapContour(inputROIPoints, inputLine, outputLines, lineSpacing);
        }
        else if (vtkVariant(GetConversionParameter(GetEndCappingParameterName())).ToInt() == EndCappingModes::Straight)
        {
            CreateStraightEndCapContour(inputROIPoints, inputLine, outputLines, lineSpacing);
        }*/
    }
        ...
        for (int currentLineIndex = 0; currentLineIndex < numberOfLines; ++currentLineIndex)
        {
            ...
            for (CappingDirection direction : CappingDirections)
            {
                ...
                if (!lineTriangulated)
                {
                    ...
                    CreateEndCapContour(inputROIPoints, currentLine, externalLines, lineOffset);
                    ...
                    for (int currentLineId = 0; currentLineId < numberOfCells; ++currentLineId)
                    {
                        ...
                        TriangulateContourInterior(newLine, outputPolygons, direction == CAPPING_ABOVE);
                    ...
                    for (int currentLineId = 0; currentLineId < numberOfCells; ++currentLineId)
                    {
                        ...
                        if (direction == CAPPING_ABOVE)
                        {
                            TriangulateBetweenContours(inputROIPoints, dividedLine->GetPointIds(), idLists[currentLineId], outputPolygons);
                        }
                        else
                        {
                            TriangulateBetweenContours(inputROIPoints, idLists[currentLineId], dividedLine->GetPointIds(), outputPolygons);
                        }
                    }
    ...
    vtkSmartPointer<vtkPolyData> Convert(vtkPolyData* planarContoursPolyData, vtkPolyData* closedSurfacePolyData)
    {
        ...
        while (firstLineOnPlane1Index + numberOfLinesInPlane1 < numberOfLines)
        {
            ...
            std::cout << "No Original line number: " << numberOfLinesInPlane1 << std::endl;
            ...
            for (int line1Index = firstLineOnPlane1Index; line1Index < firstLineOnPlane1Index + numberOfLinesInPlane1; ++line1Index)
            {
                ...
                for (size_t overlapIndex = 0; overlapIndex < plane1Overlaps[line1Index - firstLineOnPlane1Index].size(); ++overlapIndex) // lines on plane 2 that overlap with line 1
                {
                    ...
                    if (numberOfdividedPointsInLine1 > 1 && numberOfdividedPointsInLine2 > 1)
                    {
                        ...
                        TriangulateBetweenContours(inputContoursCopy, dividedPointsInLine1, dividedPointsInLine2, outputPolygons);
                    }
        ...
        std::cout << "Points number: " << outputPoints->GetNumberOfPoints() << std::endl;
        std::cout << "Polygons number: " << outputPolygons->GetNumberOfCells() << std::endl;
