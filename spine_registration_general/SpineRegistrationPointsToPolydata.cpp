#include "SpineRegistrationPointsToPolydata.hpp"
...
SpineRegistrationPointsToPolydata::SpineRegistrationPointsToPolydata(const vtkSmartPointer<vtkPolyData>& pPoly, const std::vector<PointTypeITK>& pTargetPointsOnPoly, const std::vector<PointTypeITK>& pSourceExternalPoints)
{
    mPoly = pPoly;
...
itk::Rigid3DTransform<double>::Pointer SpineRegistrationPointsToPolydata::MakeFinalAlignment(const std::vector<PointTypeITK>& pAlignmentPoints, double& pError)
{
    LeastSquaresICP myICP(pAlignmentPoints);

    pError = myICP.LeastSquaresRandomInitThreadSafe(mPoly, mTransform);
