...
namespace Test {
	...
	using ImageTypeSegmentation = itk::Image<unsigned char, 3>;
	...
	itk::Image<uint8_t, 3>::Pointer CloseImage1(ImageTypeSegmentation::Pointer pImage)
	{
		using FilterType = itk::VotingBinaryIterativeHoleFillingImageFilter<ImageTypeSegmentation>;
		FilterType::InputSizeType radius;
		radius.Fill(2);

		auto holeFillFilter = FilterType::New();

		holeFillFilter->SetInput(pImage);
		holeFillFilter->SetForegroundValue(1);
		holeFillFilter->SetBackgroundValue(0);
		holeFillFilter->SetMajorityThreshold(2);
		holeFillFilter->SetRadius(radius);
		holeFillFilter->SetMaximumNumberOfIterations(5);
		holeFillFilter->Update();
		return holeFillFilter->GetOutput();
	}

	void CloseImage2(ImageTypeSegmentation * pImage)
	{
		auto contourFilter = itk::BinaryContourImageFilter<ImageTypeSegmentation, ImageTypeSegmentation>::New();
		contourFilter->SetInput(pImage);
		contourFilter->SetForegroundValue(1);
		contourFilter->SetBackgroundValue(0);
		contourFilter->Update();

		// Crea el interpolador de spline cúbica.
		auto interpolator = itk::BSplineInterpolateImageFunction<ImageTypeSegmentation>::New();
		interpolator->SetSplineOrder(3);
		interpolator->SetInputImage(contourFilter->GetOutput());

		// Rellena los agujeros utilizando la spline cúbica.
		itk::ImageRegionIterator<ImageTypeSegmentation> it(pImage, pImage->GetRequestedRegion());
		for (it.GoToBegin(); !it.IsAtEnd(); ++it) {
			if (it.Get() == 0) {
				ImageTypeSegmentation::PointType point;
				pImage->TransformIndexToPhysicalPoint(it.GetIndex(), point);
				auto aaa = interpolator->Evaluate(point);
				if (aaa != 0)
				{
	...
	vtkSmartPointer<vtkPolyData> CloseImage3(ImageTypeSegmentation::Pointer pImage)
	{
		using ConnectorType = itk::ImageToVTKImageFilter<ImageTypeSegmentation>;
		auto connector1 = ConnectorType::New();
		connector1->SetInput(pImage);
		connector1->Update();
		
		auto surface = vtkSmartPointer<vtkFlyingEdges3D>::New();

		surface->SetInputData(connector1->GetOutput());
		...
		vtkSmartPointer<vtkClipClosedSurface> fillHolesFilter = vtkSmartPointer<vtkClipClosedSurface>::New();
		fillHolesFilter->SetInputData(poly);
