...
void FemurImplantMatch::getBorderLines(std::vector<Point>& pPoints, const Plane& currentPlane, const Line& pTopLine, const Line& pMedialLine, const Line& pLateralLine, const Line& pDownLine) const
{
	Line topLine = Line(pTopLine);
	Line medialLine = Line(pMedialLine);
	Line lateralLine = Line(pLateralLine);
	Line downLine = Line(pDownLine);
	...
	double topTemp, lateralTemp, medialTemp, downTemp;

	std::vector<Point>::const_iterator it1, it2;
	it1 = pPoints.begin();
	it2 = pPoints.end();

	Point farTop, farMedial, farLateral, farDown;
	...
	topLine.setPoint(farTop);
	downLine.setPoint(farDown);
	lateralLine.setPoint(farLateral);
	medialLine.setPoint(farMedial);
	...
	pPoints.push_back(cornerTopLat);
	pPoints.push_back(cornerTopMed);
	pPoints.push_back(cornerDownLat);
	pPoints.push_back(cornerDownMed);

	/*auto poly = ImplantTools::getContours(knee.GetFemurPoly(), currentPlane.getNormalVector(), currentPlane.getPoint());
	std::vector<Point> mainFourPoints;
	mainFourPoints.push_back(cornerTopLat);
	mainFourPoints.push_back(cornerTopMed);
	mainFourPoints.push_back(cornerDownLat);
	mainFourPoints.push_back(cornerDownMed);
	ImplantTools::show(poly, mainFourPoints);*/
}
...
std::vector<PointTypeITK> FemurImplantMatch::GetHullPoints(const itk::Rigid3DTransform<>::Pointer pTransformIn, itk::Rigid3DTransform<>::Pointer pTransformOut, PlaneID id, double distanceSide, double distanceTop, double angleLateral, double angleMedial, int amount, double middleCurveHight, CurveType pCurve, double moveWCurveCenterAngle) const
{
	...
	if (id == kPlaneA)
	{
		...
		projectedPointsIncreased = projectedPoints;
		...
		if (projectedPoints.size() > 15)
		{
			...
			Line pTopLine = Line(currentPlane.getProjectionVector(fromMedToLatVector), topPoint);
			Line pDownLine = Line(currentPlane.getProjectionVector(fromMedToLatVector), downPoint);
			Line pMedialLine = Line(currentPlane.getProjectionVector(knee.getDirectVectorFemurAxis()), medialSide);
			Line pLateralLine = Line(currentPlane.getProjectionVector(knee.getDirectVectorFemurAxis()), lateralSide);

			getBorderLines(pointsLatTemp, currentPlane, pTopLine, pMedialLine, pLateralLine, pDownLine);
			getBorderLines(pointsMedTemp, currentPlane, pTopLine, pMedialLine, pLateralLine, pDownLine);
			getBorderLines(projectedPointsIncreased, currentPlane, pTopLine, pMedialLine, pLateralLine, pDownLine);
		}
		else
		{
			return hull;
	...
	{
		if (pointsLatTemp.size() == 0 || pointsMedTemp.size() == 0 || pCurve == CurveType::KCurveU)
		{
			getCurveLikeU(projectedPointsIncreased, downPoint, lateralSide, medialSide, topPoint, midPlane, currentPlane, myRotation, vertices, distanceSide, distanceTop, amount);
		}
		else
		{
...
void FemurImplantMatch::getVerticesA(const std::vector<Point>& points, const std::vector<Point>& pointsLat, const std::vector<Point>& pointsMed, const Point& downPoint, const Point& lateralPoint, const Point& medialPoint, const Point& topPoint, const Plane& midPlane, const Plane& currentPlane, const cv::Mat& pRotation, std::vector<Point>& vertices, double distance, int amount, bool longCurve) const
{
	...
	//ImplantTools::show(knee.GetFemurPoly(), points);
	...
	lateralOut = pointsLat;
	medialOut = pointsMed;
	centerPoint = currentPlane.getProjectionPoint(knee.getFemurKneeCenter());
...
void FemurImplantMatch::getCurveLikeW(const std::vector<Point>& pointsLat, const std::vector<Point>& pointsMed, const Point& downPoint, const Point& lateralPoint, const Point& medialPoint, const Point& topPoint, const Plane& midPlane, const Plane& currentPlane, const cv::Mat& pRotation, std::vector<Point>& vertices, double distanceSide, double distanceTop, int amount, PlaneID planeID, double middleCurveHight, double moveCenterAngle) const
{
	...
	Point latGroove1, latGroove2, medGroove1, medGroove2;
	...
	if (hullFeaturesLat.medialDownPos == 0)
	{
		...
		latGroove1 = convexLat[hullFeaturesLat.medialDownPos];
		latGroove2 = convexLat[hullFeaturesLat.medialDownPos + 1];
	}
	else
	{
	...
	if (hullFeaturesMed.lateralDownPos == 0)
	{
		...
		medGroove1 = convexMed[hullFeaturesMed.lateralDownPos];
		medGroove2 = convexMed[hullFeaturesMed.lateralDownPos + 1];
	}
	else
	{
	...
	/*std::pair<int, float> sideLat = ImplantTools::GetNearestPointToLine(convexLat, lineLat, Plane());
	std::pair<int, float> sideMed = ImplantTools::GetNearestPointToLine(convexMed, lineMed, Plane());
	...
	Plane basePlane;
	basePlane.init(normalTemp, baseLine.getPoint());
	basePlane.reverseByPoint(topPoint);

	Line lineLat = Line::makeLineWithPoints(latGroove1, latGroove2);
	Line lineMed = Line::makeLineWithPoints(medGroove1, medGroove2);

	std::vector<Point> convexLatNew, convexMedNew;

	Point basePointLat = basePlane.getInterceptionLinePoint(lineLat);
	Point basePointMed = basePlane.getInterceptionLinePoint(lineMed);
	Point basePointMid = (basePointLat + basePointMed) / 2.;

	Point moveVectorLat = basePointMid - basePointLat;
	Point moveVectorMed = basePointMid - basePointMed;
	moveVectorLat.normalice();
	...
	//basePointMed = basePointMed + 0.1 * (basePointLat - basePointMed);

	int beginPosLat, endPosLat;
	int beginPosMed, endPosMed;

	if (hullFeaturesLat.medialDownPos == 0)
	{
		convexLatNew.push_back(basePointLat);

		for (int i = 1; i < convexLat.size(); i++)
		{
			convexLatNew.push_back(convexLat[i]);
		}
		//Eliminando algun punto colinial
		Point tempVector1 = convexLatNew[1] - convexLatNew[0];
		Point tempVector2 = convexLatNew[2] - convexLatNew[1];
		if (ImplantTools::getAngleBetweenVectorsDegree(tempVector1, tempVector2) < 45)
		{
			convexLatNew.erase(convexLatNew.begin() + 1);
		}
		beginPosLat = 0;
	...
	{
		for (int i = 0; i < convexLat.size() - 1; i++)
		{
			convexLatNew.push_back(convexLat[i]);
		}

		endPosLat = convexLatNew.size() - 1;
		convexLatNew.push_back(basePointLat);
		beginPosLat = endPosLat + 1;

		Point tempVector1 = convexLatNew[beginPosLat - 1] - convexLatNew[beginPosLat];
		Point tempVector2 = convexLatNew[beginPosLat - 2] - convexLatNew[beginPosLat - 1];
		if (ImplantTools::getAngleBetweenVectorsDegree(tempVector1, tempVector2) < 45)
		{
			convexLatNew.erase(convexLatNew.begin() + beginPosLat - 1);
			endPosLat -= 1;
			beginPosLat -= 1;
		}
	...
	if (hullFeaturesMed.lateralDownPos == 0)
	{
		...
		for (int i = 1; i < convexMed.size(); i++)
		{
			convexMedNew.push_back(convexMed[i]);
		}

		Point tempVector1 = convexMedNew[1] - convexMedNew[0];
		Point tempVector2 = convexMedNew[2] - convexMedNew[1];
		if (ImplantTools::getAngleBetweenVectorsDegree(tempVector1, tempVector2) < 45)
		{
			convexMedNew.erase(convexMedNew.begin() + 1);
		}
	...
	{
		for (int i = 0; i < convexMed.size() - 1; i++)
		{
			convexMedNew.push_back(convexMed[i]);
		}

		endPosMed = convexMedNew.size() - 1;
		convexMedNew.push_back(basePointMed);
		beginPosMed = endPosMed + 1;

		Point tempVector1 = convexMedNew[beginPosMed - 1] - convexMedNew[beginPosMed];
		Point tempVector2 = convexMedNew[beginPosMed - 2] - convexMedNew[beginPosMed - 1];
		if (ImplantTools::getAngleBetweenVectorsDegree(tempVector1, tempVector2) < 45)
		{
			convexMedNew.erase(convexMedNew.begin() + beginPosMed - 1);
			endPosMed -= 1;
			beginPosMed -= 1;
		}
	...
	{
		Point vectorTemp = convexLatNew[endPosLat] - convexLatNew[beginPosLat];
		vectorTemp.normalice();
		distanceFix = (distanceFix > middleCurveHight) ? middleCurveHight : distanceFix;
		convexLatNew[beginPosLat] = convexLatNew[beginPosLat] + distanceFix * vectorTemp;
	}

	if (middleCurveHight > 0)
	{
		Point vectorTemp = convexMedNew[endPosMed] - convexMedNew[beginPosMed];
		vectorTemp.normalice();
		distanceFix = (distanceFix > middleCurveHight) ? middleCurveHight : distanceFix;
		convexMedNew[beginPosMed] = convexMedNew[beginPosMed] + distanceFix * vectorTemp;
	}
...
FemurImplantMatch::ConvexHullFeatures FemurImplantMatch::getIncreaseBorder(const std::vector<Point>& points, const Point& downPoint, const Point& lateralPoint, const Point& medialPoint, const Point& topPoint, const Plane& midPlane, const Plane& currentPlane, const cv::Mat& pRotation, double distanceSideLat, double distanceSideMed, double distanceTop, double downLatCornerOut, double downMedCornerOut) const
{
	...
	/*std::vector<Point> mainFourPoints2 = { fullConvex[topLatCorner], fullConvex[topMedCorner], fullConvex[downLatCorner], fullConvex[downMedCorner] };
	ImplantTools::show(poly, mainFourPoints2);*/
	////////////////////////////////////////////////////////////
	...
	//ImplantTools::show(poly, tempTest1);
	//ImplantTools::show(poly, finalHull, true);
...
void FemurImplantMatch::getCurveLikeU(const std::vector<Point>& points, const Point& downPoint, const Point& lateralPoint, const Point& medialPoint, const Point& topPoint, const Plane& midPlane, const Plane& currentPlane, const cv::Mat& pRotation, std::vector<Point>& vertices, double distanceSide, double distanceTop, int amount) const
{
	...
	Point centerP1 = currentPlane.getProjectionPoint(knee.getFemurKneeCenter());
	Point centerP2 = currentPlane.getProjectionPoint((knee.getHipCenter() - 20 * knee.getFemurDirectVectorAP()));

	Point vectorAP = midPlane.getProjectionPoint(centerP2) - midPlane.getProjectionPoint(centerP1);
	vectorAP.normalice();
	...
	Plane obliqueLat, obliqueMed;

	obliqueLat = currentPlane.getPerpendicularPlane(centerPoint, fullConvex[topLatCorner]);
	obliqueMed = currentPlane.getPerpendicularPlane(centerPoint, fullConvex[topMedCorner]);

	obliqueLat.reverseByPoint(fullConvex[topMedCorner]);
	obliqueMed.reverseByPoint(fullConvex[topLatCorner]);
	Plane myMidPlane = midPlane;
	myMidPlane.movePlane(medPointConv);
	myMidPlane.reverseByPoint(latPointConv);
	myMidPlane.movePlane(centerPoint);
	...
	for (int i = 0; i < tSize; i++)
	{
		if (i == topLatCorner || i == topMedCorner)
		{
			...
			if (i == topLatCorner)
			{
				fullConvex[i] = fullConvex[i] +  distanceSide * myMidPlane.getNormalVector();
			}
			else
			{
				fullConvex[i] = fullConvex[i] - distanceSide * myMidPlane.getNormalVector();
		...
		{
			if (myMidPlane.eval(fullConvex[i]) > 0)
			{
				fullConvex[i] = fullConvex[i] + distanceSide * myMidPlane.getNormalVector();
			}
			else
...
void FemurImplantMatch::separateMedialAndLateralPoints(const std::vector<Point>& points, const Plane& sagitalPlane, const Point& extremeLatPoint, const Point& extremeMedPoint, std::vector<Point>& lateralOut, std::vector<Point>& medialOut, Point& centerPointOut) const
{
	...
	std::cout << "aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" << edge.latDistance << ";  " << edge.medDistance << " Suma: " << edge.latDistance + edge.medDistance << std::endl;
