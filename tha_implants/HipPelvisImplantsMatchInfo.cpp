...
double HipPelvisImplantsMatchInfo::getCupInclination(double pTiltAngle) const
{
	auto pelvisTemp = mPelvis.getHipPelvisCopy(pTiltAngle);

	Point rotationAxis = pelvisTemp.getPelvisVectorAP();
	Point referenceVector = pelvisTemp.getPelvisVectorInfSup();
	...
	Plane sagital;
	Point ref = pelvisTemp.getPubicJoin();
	sagital.init(pelvisTemp.getPelvisVectorLateralASIS(), ref);
...
double HipPelvisImplantsMatchInfo::getCupAntversion(double pTiltAngle) const
{
	auto pelvisTemp = mPelvis.getHipPelvisCopy(pTiltAngle);
	...
	Point refPoint = pelvisTemp.getCoronalPlaneAPP().getProjectionPoint(pelvisTemp.getPubicJoin());

	Point pelvisVectorAP = pelvisTemp.getPelvisVectorAP();
	...
	*/
	...
	Point interceptionVector = pelvisTemp.getCoronalPlaneAPP().getInterceptionPlaneVector(implantVector, pelvisVectorAP);
	...
	if (pelvisTemp.getCoronalPlaneAPP().eval(refPoint) < 0)
	{
		return angle;
	}
	else
...
double HipPelvisImplantsMatchInfo::getCupInclination(const itk::Rigid3DTransform<>::Pointer pImplantToBoneTransform, double pTiltAngle) const
{
	auto pelvisTemp = mPelvis.getHipPelvisCopy(pTiltAngle);
...
double HipPelvisImplantsMatchInfo::getCupAntversion(const itk::Rigid3DTransform<>::Pointer pImplantToBoneTransform, double pTiltAngle) const
{
	/*
		This vectorX represent the vector perpendicular to the base of the cup,
		which should point towards the center of the hip and not outwards,
	...
	*/
	...
	Point interceptionVector = pelvisTemp.getCoronalPlaneAPP().getInterceptionPlaneVector(implantVectorZ, pelvisVectorAP);
	interceptionVector.normalice();
...
double HipPelvisImplantsMatchInfo::getCupInclination(const Point& pVectorToHipCenter, double pTiltAngle) const
{
	auto pelvisTemp = mPelvis.getHipPelvisCopy(pTiltAngle);
...
double HipPelvisImplantsMatchInfo::getCupAntversion(const Point& pVectorToHipCenter, double pTiltAngle) const
{
	auto pelvisTemp = mPelvis.getHipPelvisCopy(pTiltAngle);
...
itk::Rigid3DTransform<>::Pointer HipPelvisImplantsMatchInfo::setCupAngles(double pAbductionAngle, double pAnteversionAngle, double pTiltAngle)
{
	auto pelvisTemp = mPelvis.getHipPelvisCopy(pTiltAngle);
...
itk::Vector< double, 3 > HipPelvisImplantsMatchInfo::setStemAxisShiftHip(double pShifSuperior, double pShifLateral, double pShiftAnterior)
{
	cv::Mat stemBasePoint = (mRotationStem * mImplantStem.getBasePoint().ToMatPoint()) + mTranslationStem;
	Point stemBase = Point(stemBasePoint);

	Point newCenterStem = mHipCenterOfRotation + pShifSuperior * mPelvis.getPelvisVectorInfSup();
	newCenterStem = newCenterStem + pShifLateral * mPelvis.getPelvisVectorLateralASIS();
	newCenterStem = newCenterStem + pShiftAnterior * mPelvis.getPelvisVectorAP();
	
	Point diff = newCenterStem - stemBase;
	mTranslationStem = mTranslationStem + diff.ToMatPoint();

	return ImplantTools::CVTranslationToITKVector(mTranslationStem);
}

itk::Vector< double, 3 > HipPelvisImplantsMatchInfo::setStemAxisShiftCup(double pShifSuperior, double pShifLateral, double pShiftAnterior)
{
	cv::Mat stemBasePoint = (mRotationStem * mImplantStem.getBasePoint().ToMatPoint()) + mTranslationStem;
	Point stemBase = Point(stemBasePoint);
...
{	
	...
	for (int i = 0; i < 10; i++)
	{
		...
		if (abs(refAngle - angle) < threshold)
		{
			break;
		}

		cv::Mat tempTransform = rotateStemVersionAngle(pStemVersionAngleDegree);
		neckAxisMat = tempTransform * mImplantStem.getVectorNeckToHead().ToMatPoint();
		neckAxis = Point(neckAxisMat);

		neckAxis.normalice();

		double angleTemp = mPelvis.getFemurVersionRadian(neckAxis);
		double diff1 = abs(angle - refAngle);
		double diff2 = abs(angle - angleTemp);
		double alpha = diff2 / diff1;
...
cv::Mat HipPelvisImplantsMatchInfo::rotateStemVersionAngle(double pStemVersionAngleDegree, double alpha)
{
	cv::Mat neckAxisMat = mRotationStem * mImplantStem.getVectorNeckToHead().ToMatPoint();
	Point neckAxis = Point(neckAxisMat);
	...
	cv::Mat rodAxis = mRotationStem * mImplantStem.getVectorInfSup().ToMatPoint();
	Point canalAxis = Point(rodAxis);
	canalAxis.normalice();
	...
	if (refAngle > angle)
	{
		double temp = alpha * (refAngle - angle);
	...
	{
		double temp = alpha * (angle - refAngle);
	...
	return transformation * mRotationStem;
}
